...
WCHAR* KFile::DigitalSignature() const
{
	...
	__try
	{
		...
		::OutputDebugString(_T("Serial Number: "));
...
bool KFile::GetFileVersion( const wstring& fileName, DWORD *pdwFileVersionMS, DWORD *pdwFileVersionLS )
{
	...
	DWORD dwSize;
	dwSize=GetFileVersionInfoSizeW(fileName.c_str(), dwHandle);
	if (dwSize==0)
		return false;

	wchar_t* pData=new wchar_t[dwSize];

	if (!GetFileVersionInfoW(fileName.c_str(), NULL, dwSize, pData))
	{
		delete[] pData;
		return false;
	}
	...
	VerQueryValueW(pData, L"\\", (LPVOID*)&pVerInfo, &uSize);
...
bool KFile::Copy( const wstring& sourceFileName, const wstring& destFileName, bool overwrite /*= true*/ )
{
	...
	if (overwrite)
	{
		return (TRUE == ::CopyFileW(sourceFileName.c_str(), destFileName.c_str(), FALSE));
	}

	if (::CopyFileW(sourceFileName.c_str(), destFileName.c_str(), TRUE))
	{
		return true;
	}
	else
...
bool KFile::Exists( const wstring& path )
{
	...
	return (TRUE == ::PathFileExistsW(path.c_str()));
}
...
bool KFile::Rename( const wstring& oldPath, const wstring& newPath, bool replaceWhenExist)
{
    ...
    //if (!KPath::Exists(oldPath))
    //{
    //    return false;
    //}

    //if (KPath::Exists(newPath) && replaceWhenExist)
    //{
...
string KFile::ReadAllTextToAnsi( const wstring& path ){
    if (path.empty())
    {
        return false;
    }
...
wstring KFile::ReadAllText( const wstring& path, bool isUTF8 /*=true*/ )
{
	...
	if (isUTF8)
	{
		...
		allText = cy::convert::StringConvert::Utf8ToWide(ansiAllText);
	}
	else
	{
		allText = cy::convert::StringConvert::AnsiToWide(ansiAllText);
	}
...
bool KFile::WriteToDisk(const char* output_file,int nlen,char* buff)
{
	...
	if(fwrite(buff,1,nlen,outfile) != nlen)
	{
		return false;
	}
	fclose(outfile);
...
bool KFile::GetFileSize( const wstring& path, LARGE_INTEGER& fileSize )
{
    if (path.empty())
    {
        return false;
