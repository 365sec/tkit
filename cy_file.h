...
namespace cy
{
			namespace io
			{
				...
				public:
					...
					/**
                    * @brief   将缓冲区里的内容写入文件
                    * @param   const char* output_file  文件路径
                    * @param  char* buff  缓冲区的大小
					* @param  char* buff  缓冲区
                    * @return  bool
                    * @note    支持4G以上文件
                    */
					static bool WriteToDisk(const char* output_file,int nlen,char* buff);
                    ...
                    */
					 ...
					 * @brief 读一个文本文件的所有内容
					 * @param const wstring & path 文件路径
					 * @return std::string 文件的内容
					 * @note 
					 */
					static string ReadAllTextToAnsi( const wstring& path );
